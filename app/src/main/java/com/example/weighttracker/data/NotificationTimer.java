...
public class NotificationTimer {
    ...
    private class Work extends TimerTask
    {
        ...
        public Work(Context context) {
            this.context = context;
            this.dataSource = new weightDataSource(this.context);
        }
        ...
        public void run()
        {
            ...
            String selection = SqlDbContract.weightEntry.COLUMN_NAME_COUNT + " < ?";
            String[] selectionArgs = { "5" };
            // Query
            Cursor cursor = dataSource.queryweightDatabase(projection, selection, selectionArgs, null);
            // Iterate through the cursor
            for (HashMap row : SqlDbHelper.GetAll(cursor)) {
                // Get Product Name and Count
                String productName = row.get(SqlDbContract.weightEntry.COLUMN_NAME_NAME).toString();
                String productCount = row.get(SqlDbContract.weightEntry.COLUMN_NAME_COUNT).toString();
